...
// SearchAvailaibilityByDatesByRoomID returns true if availability exists for roomID, and false if no availability exists
func (m *postgresDBRepo) SearchAvailaibilityByDatesByRoomID(start, end time.Time, roomID int) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
// SearchaAvailabilityFromAllRooms returns a slice of available rooms for a given dates
func (m *postgresDBRepo) SearchaAvailabilityFromAllRooms(start, end time.Time) ([]models.Room, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	...
	query := `select UD, first_name, last_name, email, password, access_level, created_at, updated_at
